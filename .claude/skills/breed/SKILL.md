...
ユーザーからの次のメッセージに添付された画像 path (会話履歴に残る一時パス) を Read ツールで読み込んで vision 抽出に進む。1 枚しか無い / 関係ない画像の場合はもう一度添付を依頼する。

### 2. 対象ポケモン選択

2 枚から 6 体のポケモン名を vision で読み取り、AskUserQuestion で「育成対象とするポケモンはどれですか?」と提示する (6 体から 1 体選択)。

### 3. Vision 抽出

選ばれた 1 体について、2 枚の画像から以下を vision で抽出する:
