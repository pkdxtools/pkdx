...
```
Champions のスクショ 2 枚を、次のメッセージにまとめて貼ってください:
 - 「能力」画面 (特性・持ち物・技 4 つ)
 - 「ステータス」画面 (SP・実数値・性格 ↑↓)

※ チーム画面 (6 体並んだもの) を貼った場合、対象とするポケモンを後から指定できます。
```

ユーザーからの次のメッセージに添付された画像 path (会話履歴に残る一時パス) を Read ツールで読み込んで vision 抽出に進む。1 枚しか無い / 関係ない画像の場合はもう一度添付を依頼する。チーム画面 (6 体並び) の場合は、抽出後にどの体を対象とするか AskUserQuestion で選ばせる。
...
| # | 質問 | header | オプション | multiSelect |
|---|------|--------|-----------|-------------|
| 1 | この抽出でよいですか? | 確認 | はい(desc: cache 組み立てに進む), 修正する(desc: 項目を個別に修正), やり直す(desc: スクショ添付から再依頼) | false |
