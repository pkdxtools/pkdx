...
抽出した 6 体について、breed skill の [Phase 1-Vision](../breed/SKILL.md#phase-1-vision-champions-スクショ取り込み) 手順 3-6 (DB 照合 / SP 逆算検証 / 性格確定 / 技詳細補完) を順次適用する。これらは個別 CLI 呼び出しが必要なため**ポケモンごと**に処理するが、ユーザー対話は発生させず一括で進める (性格が一意に決まらない体だけ、後段の確認時にまとめて選ばせる)。
