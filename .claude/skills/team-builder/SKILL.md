...
| Phase 1-Team-Vision 確認後 | 6 体一括取り込み後にまとめて入力 |
...
確認 OK の後、6 体それぞれの `role`（ポケモンメモ）を順に質問する。AskUserQuestion ではなく [メンバー確定共通: ポケモンメモ入力](#メンバー確定共通-ポケモンメモ入力) のフリー入力フローを使い、`members[0]` から順に依頼する。1 体ずつではなく「6 体まとめて行ごとに記入してもらう」形式でも可（`1: ...\n2: ...` の形で 1 メッセージ受領 → 行頭番号で各 `role` に割り振り）。
