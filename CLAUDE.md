...
役割論理（やくわりろんり）流派の支援スキル。CLI 風のサブコマンドで 3 機能に分岐する:

- **`y matrix`** — 役割遂行マトリクス計算。対象ヤケモンとメタ上位 N 体の往復ダメージを `pkdx damage` で総合し、完全役割／削り役／受け特化／論外を判定する。
- **`y verify`** — ヤーティ正当性検証。`box/teams/*.meta.json` を読み取り専用で検査し、メンバー単位で ヤトリック／ヤロテスタント／論外 を判定する。
- **`y logic`** — ロジカル語法モード（Mode A）のトグル。役割論者風の文体（〜ですぞｗｗｗ／我／貴殿／総合的にロジック等）で応答する状態を会話セッション内で ON/OFF する。

サブコマンドはスキル起動時の引数、またはユーザー発話のキーワード（「マトリクス」「ヤーティ検証」「ロジカル語法」等）から判別する。曖昧な場合は AskUserQuestion で確認する。

**references**: 教義 SSoT（`doctrine.md`）、ロジカル語法コーパス（`logical_grammar.md`）、判定規則（`validation_rules.md`）、メタ上位テンプレート（`meta_template.md`）は `.claude/skills/y/references/` に集約。

**運用原則**: Claude の既存知識で役割論理を一次解答してはならない。Mode A は会話セッション内でのみ保持され、別セッションに持ち越さない。`y verify` は team-builder の出力を一切改変しない。
