...
| ダメージ計算 | テラスタル | ✅ | |
| ダメージ計算 | 天候・フィールド補正 | ⚠️ | `pkdx damage` 単体と回復技 (つきのひかり等) / EOT (Sand chip / 天候 tick) は天候反映。状態遷移内の damage 計算は state.weather → CalcInput 未配線。フィールド (エレキ/グラス/ミスト/サイコ) は全面未対応 |
...
| 特性 | 回復・状態異常干渉 | ✅ | ふみん / やるき / スイートベール (Sleep / あくび 免疫 + Rest 失敗) / マジックガード (やどりぎ tick 無効化) / ヘドロえき (Drain + やどりぎ 反転) / ぼうじん + すなおこし / すなかき / すながくれ / すなのちから (Sand chip 免疫) |
